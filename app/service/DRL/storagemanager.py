...
class StorageManager:
    def __init__(self, name, load_session, load_episode, device):
        ...
        self.machine_dir = os.path.join(current_directory, '../../../models', str(socket.gethostname()))
        if 'examples' in load_session:
            self.machine_dir = os.path.join(current_directory, '../../../models/example', str(socket.gethostname()))
        self.name = name
    ...
    def network_save_weights(self, network, model_dir, episode):
        filepath = os.path.join(model_dir, str(network.name) + 'episode'+str(episode)+'.pt')
    ...
    def save_session(self, episode, networks, pickle_data, replay_buffer):
        ...
        # Store graph data
        with open(os.path.join(self.session_dir, 'episode'+str(episode)+'.pkl'), 'wb') as f:
            pickle.dump(pickle_data, f, pickle.HIGHEST_PROTOCOL)

        # Store latest buffer (can become very large, multiple gigabytes)
        with open(os.path.join(self.session_dir, 'latest_buffer.pkl'), 'wb') as f:
        ...
        if (episode % 1000 == 0):
            for i in range(episode, episode - 1000, 100):
                for network in networks:
                    self.delete_file(os.path.join(self.session_dir, network.name +'episode'+str(i)+'.pt'))
                self.delete_file(os.path.join(self.session_dir, +'episode'+str(i)+'.pkl'))

    def store_model(self, model):
        with open(os.path.join(self.session_dir, 'agent.pkl'), 'wb') as f:
            pickle.dump(model, f, pickle.HIGHEST_PROTOCOL)

    # ------------------------------- LOADING -------------------------------

    def network_load_weights(self, network, model_dir, stage, episode):
        filepath = os.path.join(model_dir, str(network.name) + '_stage'+str(stage)+'episode'+str(episode)+'.pt')
    ...
    def load_graphdata(self):
        with open(os.path.join(self.session_dir, +'episode'+str(self.load_episode)+'.pkl'), 'rb') as f:
    ...
    def load_model(self):
        model_path = os.path.join(self.session_dir, 'agent.pkl')
        try :
